...
const SocialLinks = () => {
  ...
  const socialLinksCollection = React.useMemo(
    () => (firestore ? collection(firestore, 'social-links') : null),
    [firestore]
  );

  const { data: socialLinks, isLoading } =
    useCollection<SocialLink>(socialLinksCollection);
...
export default function Footer() {
  const [copyrightText, setCopyrightText] =
    React.useState('');

  const firestore = useFirestore();

  const brandingDocRef = React.useMemo(
    () => (firestore ? doc(firestore, 'branding', 'main') : null),
    [firestore]
  );

  const { data: branding, isLoading: isLoadingBranding } =
    useDoc<Branding>(brandingDocRef);

  React.useEffect(() => {
    setCopyrightText(
      `© ${new Date().getFullYear()} Asociación cultural Ateaco. Todos los derechos reservados.`
    );
  }, []);
